...
| `github-actions-ec2-pipeline` | Set `PROD_URL` for the health-check workflow, then capture EC2 `/api/health`, PM2 process status, and one rollback log with sensitive values hidden |
